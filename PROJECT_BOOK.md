...
3. **Deploy to Staging** (only on push to `main`, and only if the two previous jobs passed) — builds the backend and frontend images, pushes to ECR, then **updates the image tag in `values-staging.yaml` in Git itself** (never touches the cluster directly!) — this is the "trigger" ArgoCD detects and syncs to staging.
