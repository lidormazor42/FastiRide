...
| **Alembic** | ניהול מיגרציות סכימה ל-Postgres — כל שינוי סכימה הוא קובץ מיגרציה מגורסן ב-Git (upgrade+downgrade), רץ אוטומטית דרך `initContainer` לפני שהבקנד עולה, החליף מנגנון `ALTER TABLE IF NOT EXISTS` ידני |
...
**מיגרציה ל-RDS.** במקור Postgres רץ כ-StatefulSet בתוך הקלאסטר (כדי לחסוך עלות בשלב הפיתוח המהיר). לקראת ההגשה, בוצעה מיגרציה מלאה ל-RDS מנוהל: מודול Terraform חדש, סיסמה שנוצרת אקראית ונשמרת ב-SSM Parameter Store (לא בקוד), עדכון סקריפטי ה-bootstrap/teardown, והסרת ה-StatefulSet מה-Helm chart. אומת מקצה לקצה: 7 הטבלאות נוצרו נכון על ה-RDS (בתחילה דרך מנגנון ה-`ALTER TABLE IF NOT EXISTS` הפנימי, מאז הוחלף ב-Alembic — ראו הפריט הבא).

**מ-`ALTER TABLE IF NOT EXISTS` ל-Alembic אמיתי.** במשך רוב הפרויקט השינויים בסכימה נוצרו על ידי בלוק קוד ב-`main.py` שהריץ `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` בכל עליית אפליקציה — עבד, אבל בלי היסטוריית גרסאות, בלי אפשרות rollback, ובלי שלב בדיקה/review לפני שהשינוי פוגע ב-DB אמיתי. כשנשאלתי במפורש אם זה מספיק טוב לפרויקט DevOps, התשובה הכנה הייתה שלא — וזו נקודת תורפה אמיתית, לא רק "פוליש". התיקון: `backend/migrations/` עם Alembic מחובר למודלים האמיתיים ול-`DATABASE_URL`; מיגרציית baseline שנוצרה אוטומטית מול DB ריק ואומתה (`alembic check`) שהיא **זהה לחלוטין** למה שהמנגנון הישן כבר בנה בפועל; `initContainer` חדש בשם `migrate` על ה-Deployment של הבקנד שמריץ `alembic upgrade head` לפני שהקונטיינר הראשי עולה — עם אותה תמונה, אותם secrets, ותחת אותו `securityContext` מוקשח (readOnlyRootFilesystem, non-root). אומת אמיתי בשתי הכיוונים: מיגרציה חדשה שמוסיפה עמודה (`rides.notes`) נוצרה, נבדקה מקומית, ופרסה עד ה-RDS האמיתי של staging (אומת ב-`psql` שהעמודה קיימת) — ואז מיגרציית reverse הסירה אותה באותו אופן. זו לא רק תיאוריה: זה מנגנון שהוכח שעובד end-to-end על תשתית אמיתית, בשני הכיוונים.

**GitOps skew — שלישית, והכי כואבת (Alembic).** בדיוק כשה-`initContainer` החדש הצטרף ל-chart, הועלה קלאסטר חדש לגמרי מאפס (טרפורם + `bootstrap-prod.sh`) — וב-sync הראשון של ArgoCD ל-prod, ה-Deployment קיבל את ה-chart החדש (עם `migrate`) אבל image.tag נשאר מקובע לגרסה הישנה שאין בה בכלל את `alembic` (הותקן רק בתמונה החדשה). התוצאה: **שני replicas של הבקנד ב-prod נתקעו ב-`Init:CrashLoopBackOff` עם `exec: "alembic": executable file not found`** — הפעם בלי אף pod ישן בריא לגבות, כי כל ה-Deployment נוצר טרי. זה בדיוק אותו דפוס שכבר קרה פעמיים (probe path, פורט nginx), אבל הפעם עם אפס replicas עובדים במקום degradation הדרגתית. תוקן זהה לקודמים: תיוג גרסה (`v0.3.0`) שמקדם את ה-image המתאים מ-staging, ועם `argocd app sync` ידני כדי לא להמתין ל-poll interval של כ-3 דקות בזמן שה-prod למטה. **המסקנה המתחזקת:** שינוי chart שמניח יכולת מסוימת בתוך ה-image (בינארי, endpoint, פורט) חייב קידום גרסה **לפני או מיד אחרי** כל sync ראשון על קלאסטר חדש — לא לחכות ל-CrashLoopBackOff כדי לגלות.
...
ב-AWS SSM Parameter Store, כ-SecureString: סיסמת RDS, מפתח חתימת ה-sessions, סיסמת Grafana, ומאז שלב הגימור גם Google OAuth client ID/secret. הרוב נוצרים ישירות על ידי Terraform (`random_password`); ה-Google OAuth הם היוצא מן הכלל — לא ניתנים ליצירה אוטומטית כי הם מגיעים מ-Google Cloud Console, ולכן מוזרקים ל-Terraform דרך `secrets.auto.tfvars` מקומי (ב-`.gitignore`, לא בגיט אף פעם) ומשם ל-SSM כמו כל השאר. עד שלב הגימור הם היו יוצאים מהכלל האמיתי — עדיין נקראו מ-`.env` ישירות ב-`bootstrap-prod.sh`, סטייה מהעיקרון הארכיטקטוני שהצהרתי עליו. תוקן כדי שכל הסודות יעברו באותו נתיב אחיד. הכל נקרא על ידי סקריפט ה-bootstrap שבונה מהם Kubernetes Secrets. שום סוד לא נמצא ב-Git — ה-`.env` המקומי ב-`.gitignore`, וההרשאות בקלאסטר מבוססות IRSA (תפקיד IAM לכל ServiceAccount) בלי מפתחות סטטיים.
...
**איך שינוי בסכימת ה-DB מגיע לפרודקשן?**
דרך Alembic, לא ידנית. כל שינוי בסכימה (`models.py`) מקבל קובץ מיגרציה מגורסן (`alembic revision --autogenerate`), עם פונקציית `upgrade()` ו-`downgrade()`, נבדק מקומית, ועובר את אותו GitOps flow כמו כל שינוי קוד אחר. ב-Kubernetes, `initContainer` בשם `migrate` על ה-Deployment של הבקנד מריץ `alembic upgrade head` **לפני** שהקונטיינר הראשי עולה — כך שאף replica לא מתחיל לשרת תעבורה מול סכימה לא-תואמת. בעבר זה נעשה עם בלוק `ALTER TABLE IF NOT EXISTS` ידני ב-`main.py` — עבד, אבל בלי היסטוריית גרסאות ובלי rollback; הוחלף מודעת ביוזמתי, אחרי שהערכתי בכנות שזו נקודת תורפה אמיתית לפרויקט DevOps.
