...
**Pipeline hardening pass:** every job across both workflows now declares explicit least-privilege `permissions` instead of relying on the repo default; `deploy-staging` has a `concurrency` group so two fast pushes to `main` queue instead of racing on the same `values-staging.yaml` commit; `promote-to-production` targets a **GitHub Environment** (`production`) with a required-reviewer rule, so pushing a version tag is no longer the final approval by itself — it also requires an explicit human click in GitHub before anything reaches `values-prod.yaml`; and `.github/dependabot.yml` now opens automatic PRs for backend pip dependencies, both Dockerfiles' base images, and GitHub Actions versions — it found two real upgrades (Python and nginx base images) on its very first scan. **First real test of the gate was inconclusive, the second wasn't.** Tagging `v0.3.1` immediately after creating the `production` environment ran straight through with no pause at all — worth noting since the API does report `can_admins_bypass: true` on this repo (repository owners can bypass required reviewers, a personal-repo limitation with no setting to disable it), which looked at first like the explanation. But tagging `v0.3.2` shortly after — same repo, same owner, nothing reconfigured — genuinely paused in `pending_deployments`, required an explicit approval, and only proceeded once approved. That rules out a permanent bypass as the real explanation for the first case; the far more likely cause is that the protection rule hadn't finished propagating in the few seconds between creating it via the API and the first tag push. The gate does work for real, consistently, once it's actually live — the first attempt just happened to land in that propagation window.
...
**Dependabot's first run immediately caught a real, mergeable-looking bump that actually breaks the build.** Enabling `.github/dependabot.yml` opened 12 PRs on its very first scan. Ten were genuinely safe (5 GitHub Actions version bumps, 5 Python dependency bumps) — CI passed and each was verified with the full local test suite before merging. But the `python:3.12-slim` → `3.14-slim` bump showed the exact same green CI as the others, and merging it would have **broken the Docker build entirely**: `psycopg2-binary==2.9.9` has no prebuilt wheel for Python 3.14 yet, and the slim image has no `pg_config`/build toolchain to compile it from source. CI's own test job never catches this class of bug — it installs `requirements.txt` and runs pytest directly on a hardcoded Python 3.11 runner, never actually building either Dockerfile, so a base-image bump can look fully green while silently not being exercised at all. Caught only by actually building the image locally before merging; closed instead of merged, pending a `psycopg2-binary` upgrade compatible with 3.14. The nginx `1.27` → `1.31` bump, by contrast, was verified for real (built, and started as non-root under the same `emptyDir`+`fsGroup` constraints the real Pod uses) and merged.

**Merging several dependency PRs within seconds of each other exposed a real gap in the concurrency fix above.** The `deploy-staging` concurrency group correctly serialized the *job runs* against each other — but each run's own `git push` (bumping `values-staging.yaml`) still got rejected as non-fast-forward, because a *later* PR's merge commit kept landing on `main` while the current run was still mid-build (1-3 minutes for a docker build, plenty of time for several more merges to land). Concurrency only protects a job against copies of itself; it does nothing for commits arriving from anywhere else. Fixed by making the bump step retry with a fetch+rebase loop instead of a bare `git push` — the correct general fix for this class of race, not just a one-off patch for this specific rapid-merge scenario.
